type ServiceType = {
  name: string;
  male: { neg: number; neu: number; pos: number };
  female: { neg: number; neu: number; pos: number };
  service: { neg: number; neu: number; pos: number };
};

const SentimentByGender = ({ services }: { services: ServiceType[] }) => {
  return (
    <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200 col-span-1 xl:col-span-2">
      <h3 className="text-lg font-semibold text-gray-800 mb-4">
        Overall Sentiment by Gender
      </h3>
      <div className="space-y-3">
        <div className="flex items-center gap-4 text-xs">
          <span className="flex items-center gap-2">
            <div className="w-4 h-4 rounded bg-blue-500"></div>
            <span className="text-gray-600">Male</span>
          </span>
          <span className="flex items-center gap-2">
            <div className="w-4 h-4 rounded bg-pink-500"></div>
            <span className="text-gray-600">Female</span>
          </span>
          {/* Sentiment Legends */}
          <span className="flex items-center gap-4 text-xs ml-auto">
            <span className="flex items-center gap-1">
              <div className="w-3 h-3 rounded bg-red-400"></div>
              <span className="text-gray-600">Neg</span>
            </span>
            <span className="flex items-center gap-1">
              <div className="w-3 h-3 rounded bg-yellow-400"></div>
              <span className="text-gray-600">Neu</span>
            </span>
            <span className="flex items-center gap-1">
              <div className="w-3 h-3 rounded bg-green-400"></div>
              <span className="text-gray-600">Pos</span>
            </span>
          </span>
        </div>
        {services.map((service, idx) => (
          <div key={idx} className="space-y-1">
            <div className="text-xs text-gray-700 font-medium">
              {service.name}
            </div>
            {/* Male Bar (Darker shades) */}
            <div className="flex h-4 rounded-full overflow-hidden bg-gray-200">
              <div
                style={{ width: `${service.male.neg}%` }}
                className="bg-red-400"
              ></div>
              <div
                style={{ width: `${service.male.neu}%` }}
                className="bg-yellow-400"
              ></div>
              <div
                style={{ width: `${service.male.pos}%` }}
                className="bg-green-400"
              ></div>
            </div>
            {/* Female Bar (Lighter shades for contrast) */}
            <div className="flex h-4 rounded-full overflow-hidden bg-gray-200">
              <div
                style={{ width: `${service.female.neg}%` }}
                className="bg-red-300"
              ></div>
              <div
                style={{ width: `${service.female.neu}%` }}
                className="bg-yellow-300"
              ></div>
              <div
                style={{ width: `${service.female.pos}%` }}
                className="bg-green-300"
              ></div>
            </div>
          </div>
        ))}
        <div className="flex justify-between text-xs text-gray-400 pt-2">
          <span>0%</span>
          <span>25%</span>
          <span>50%</span>
          <span>75%</span>
          <span>100%</span>
        </div>
      </div>
    </div>
  );
};

export default SentimentByGender;
